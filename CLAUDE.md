...
**ข้อกำหนดที่มักถูกมองข้าม:** `prerequisites` เก็บเป็น array ของ `id` แต่การจะเรนเดอร์เป็นลิงก์ได้ต้องมีตาราง `id → url` ซึ่งต้องอ่านทุกไฟล์ตอน build ทำเป็น Docusaurus plugin ที่ inject `globalData` — วางแผนไว้ตั้งแต่ต้น อย่าไป hardcode ทีหลัง
...
slug: how-computers-think          # URL ถาวร ห้ามเปลี่ยนหลัง publish
...
| `slug` | **เพิ่มใหม่** | ล็อก URL ถาวรแยกจากชื่อไฟล์ ทำให้จัดลำดับบทใหม่ได้โดยไม่ทำลายลิงก์ |
...
### งบขนาดไฟล์ — แก้ตัวเลขให้สมจริงแล้ว

งบเดิม "< 200KB รวมทุกอย่าง" **เป็นไปไม่ได้** เพราะลำพัง JS ของ Docusaurus classic ก็อยู่ราว 150KB gzipped และฟอนต์ไทย 4 น้ำหนักอีก 250KB+ งบใหม่แยกเป็นสองก้อน:

| ก้อน | งบ | หมายเหตุ |
|---|---|---|
| JS + CSS | < 180KB gzipped | ไม่รวม Pyodide / sql.js / KaTeX |
| ฟอนต์ | < 150KB | 2 น้ำหนัก self-host — ถ้าเกินให้ตัดน้ำหนัก ไม่ใช่ขยายงบ |
| Pyodide, sql.js, KaTeX | ไม่นับ | lazy load ทั้งหมด |

- Lighthouse Performance ≥ 90 บนมือถือ
...
- KaTeX CSS ต้องโหลดเฉพาะหน้าที่มีสูตรจริง ไม่ใช่ทั้งไซต์
